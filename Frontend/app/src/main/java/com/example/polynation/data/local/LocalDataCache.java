...
public class LocalDataCache {
    ...
    private static final String KEY_ACHIEVEMENTS_PREFIX = "achievements_";
    private static final String KEY_VISIT_IMAGES_PREFIX = "visit_images_";
    ...
    public static void saveVisitImages(Context context, int visitPointId, List<VisitImage> images) {
        String json = new Gson().toJson(images);
        getPrefs(context).edit().putString(KEY_VISIT_IMAGES_PREFIX + visitPointId, json).apply();
    }

    public static List<VisitImage> getVisitImages(Context context, int visitPointId) {
        String json = getPrefs(context).getString(KEY_VISIT_IMAGES_PREFIX + visitPointId, null);
        if (json == null) return null;
        Type type = new TypeToken<List<VisitImage>>(){}.getType();
        return new Gson().fromJson(json, type);
    }

    public static void saveAchievementIds(Context context, int userId, List<Long> ids) {
        String json = new Gson().toJson(ids);
        getPrefs(context).edit().putString(KEY_ACHIEVEMENTS_PREFIX + userId, json).apply();
    }

    public static List<Long> getAchievementIds(Context context, int userId) {
        String json = getPrefs(context).getString(KEY_ACHIEVEMENTS_PREFIX + userId, null);
        if (json == null) return null;
