...
import com.example.polynation.presentation.assistant.AssistantOverlay;
...
public abstract class BaseNavigationActivity extends BaseActivity {
    ...
    public void tourNavigateTo(Class<?> targetActivity) {
    ...
    protected void notifyAssistantScreenShown(String screenKey) {
        View content = findViewById(android.R.id.content);
        if (content != null) {
            content.post(() -> AssistantManager.onScreenShown(this, screenKey));
        }
    }

    @Override
    public void onBackPressed() {
        AssistantOverlay overlay = AssistantOverlay.find(this);
        if (overlay != null && overlay.isCoach()) {
            AssistantManager.skip(this);
            return;
        }
        super.onBackPressed();
    }
